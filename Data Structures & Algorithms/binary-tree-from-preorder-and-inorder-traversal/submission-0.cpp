...
class Solution {
    TreeNode* build(vector<int>& preorder,int prestart,int preend,vector<int>&inorder,int instart,int inend,unordered_map<int,int>&mp){
        ...
        root->left=build(preorder,prestart+1,prestart+size,inorder,instart,inroot-1,mp);
        root->right=build(preorder,prestart+size+1,preend,inorder,inroot+1,inend,mp);
